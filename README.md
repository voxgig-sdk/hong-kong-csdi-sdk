...
The Hong Kong Common Spatial Data Infrastructure (CSDI) Portal provides access to a range of geospatial data and applications aimed at supporting data sharing and government initiatives.

Learn more about Hong Kong CSDI API at [portal.csdi.gov.hk](https://portal.csdi.gov.hk).

This is an unofficial SDK for the Hong Kong CSDI public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — Dataset and OgcService — that you
...
| --- | --- | --- |
| **Dataset** | The Dataset entity (list, load). | `/datasets` |
| **OgcService** | The OgcService entity (load). | `/map/wms` |
...
$datasets = $client->Dataset()->list();
print_r(array_map(fn($item) => $item->data_get(), $datasets));
