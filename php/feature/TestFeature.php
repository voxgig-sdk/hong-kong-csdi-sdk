...
// HongKongCsdi SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class HongKongCsdiTestFeature extends HongKongCsdiBaseFeature
{
    ...
    public function init(HongKongCsdiContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (HongKongCsdiContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = HongKongCsdiParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (HongKongCsdiContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(HongKongCsdiContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = HongKongCsdiParam::call($ctx, $k);
