...
// HongKongCsdi SDK retry feature
...
class HongKongCsdiRetryFeature extends HongKongCsdiBaseFeature
{
    ...
    public function init(HongKongCsdiContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (HongKongCsdiContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(HongKongCsdiContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(HongKongCsdiContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
