...
class HongKongCsdiMakeError
{
    public static function call(?HongKongCsdiContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new HongKongCsdiContext([], null);
        }
        $op = $ctx->op ?? new HongKongCsdiOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new HongKongCsdiResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof HongKongCsdiError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new HongKongCsdiError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof HongKongCsdiError) {
