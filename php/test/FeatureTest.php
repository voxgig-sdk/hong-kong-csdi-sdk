...
class FtCtrl extends HongKongCsdiControl
{
...
class FtClient
{
    ...
    private HongKongCsdiUtility $utility;

    public function __construct(HongKongCsdiUtility $utility, array $options)
    {
    ...
    public function get_utility(): HongKongCsdiUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (HongKongCsdiContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public HongKongCsdiContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (HongKongCsdiContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = HongKongCsdiConfig::make_feature($name);
    ...
    private static function buildUrl(HongKongCsdiSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof HongKongCsdiError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new HongKongCsdiResponse($fetched);
        }
    ...
    private function fail(HongKongCsdiContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (HongKongCsdiContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
