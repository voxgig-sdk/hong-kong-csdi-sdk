...
class DatasetEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = HongKongCsdiSDK::test($seed, null);
        $seen = iterator_to_array($base->Dataset(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = HongKongCsdiConfig::shared_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = HongKongCsdiSDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
            foreach ($sdk->Dataset(null)->stream("list", null, null) as $item) {
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set HONG_KONG_CSDI_TEST_DATASET_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function dataset_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("HONG_KONG_CSDI_TEST_DATASET_ENTID");
