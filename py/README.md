# HongKongCsdi Python SDK



The Python SDK for the HongKongCsdi API — an entity-oriented client following Pythonic conventions.
...
release tag (`py/vX.Y.Z`, see [Releases](https://github.com/voxgig-sdk/hong-kong-csdi-sdk/releases)) or
...
client = HongKongCsdiSDK({
    "apikey": os.environ.get("HONG_KONG_CSDI_APIKEY"),
})
```
...
```python
client = HongKongCsdiSDK.test(testopts, sdkopts)
```

Creates a test-mode client with mock transport. Both arguments may be `None`.

### HongKongCsdiSDK methods
...
| `apiEndpoints` | `dict` | Available API endpoints for this dataset |
...
| `lastUpdated` | `str` | Date when the dataset was last updated |
| `license` | `str` | License information for the dataset |
| `provider` | `str` | Data provider organization |
| `publishedDate` | `str` | Date when the dataset was published |
| `spatialExtent` | `dict` | Spatial extent of the dataset |
| `theme` | `str` | Framework Spatial Data Theme |
...
```python
ogc_service = client.OgcService().load({"request": "request", "service": "service", "version": "version"})
```
...
```
py/
├── hongkongcsdi_sdk.py         -- Main SDK module
...
The main module (`hongkongcsdi_sdk`) exports the SDK class.
