...
The Python SDK for the HongKongCsdi API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Dataset(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.HongKongCsdi(None).load(
    {"id": "test01"}, None
)
