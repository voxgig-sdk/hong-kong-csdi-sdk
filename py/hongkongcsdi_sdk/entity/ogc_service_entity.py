...
from hongkongcsdi_sdk.hongkongcsdi_types import (
    OgcService,
    OgcServiceLoadMatch,
)
...
class OgcServiceEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return OgcServiceEntity(self._client, opts)
    ...
    def load(self, reqmatch=None, ctrl=None) -> OgcService:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.OgcService().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
