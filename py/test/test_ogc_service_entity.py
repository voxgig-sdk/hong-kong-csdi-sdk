    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set HONGKONGCSDI_TEST_OGC_SERVICE_ENTID JSON to run live")
        client = setup["client"]
...
def _ogc_service_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("HONGKONGCSDI_TEST_OGC_SERVICE_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("HONGKONGCSDI_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("HONGKONGCSDI_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("HONGKONGCSDI_TEST_EXPLAIN") == "TRUE",
        "live": _live,
