...
class TestDatasetEntity:
    ...
    def test_should_stream(self):
        ...
        base = HongKongCsdiSDK.test(seed, None)
        seen = list(base.Dataset(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = HongKongCsdiSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Dataset(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set HONG_KONG_CSDI_TEST_DATASET_ENTID JSON to run live")
        client = setup["client"]
...
def _dataset_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "HONG_KONG_CSDI_TEST_DATASET_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("HONG_KONG_CSDI_TEST_DATASET_ENTID"))
