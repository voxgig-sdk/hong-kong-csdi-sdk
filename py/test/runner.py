...
class HongKongCsdiTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            HongKongCsdiTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = HongKongCsdiTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = HongKongCsdiTestRunner.getenv("HONG_KONG_CSDI_TEST_LIVE")
        override = HongKongCsdiTestRunner.getenv("HONG_KONG_CSDI_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = HongKongCsdiTestRunner.getenv(key)
        ...
        explain = HongKongCsdiTestRunner.getenv("HONG_KONG_CSDI_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["HONG_KONG_CSDI_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if HongKongCsdiTestRunner._test_control is not None:
            return HongKongCsdiTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                HongKongCsdiTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            HongKongCsdiTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = HongKongCsdiTestRunner.load_test_control()
...
def load_env_local():
    HongKongCsdiTestRunner.load_env_local()


def env_override(m):
    return HongKongCsdiTestRunner.env_override(m)


def entity_data(v):
    return HongKongCsdiTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return HongKongCsdiTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return HongKongCsdiTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return HongKongCsdiTestRunner.load_test_control()


def live_client_options():
    return HongKongCsdiTestRunner.live_client_options()


def live_delay_ms():
    return HongKongCsdiTestRunner.live_delay_ms()
