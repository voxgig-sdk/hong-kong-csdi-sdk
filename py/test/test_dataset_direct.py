...
class TestDatasetDirect:
    ...
    def test_should_direct_list_dataset(self):
        setup = _dataset_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-dataset", "live" if setup["live"] else "unit")
        if _skip:
