...
describe("DatasetEntity", function()
  ...
  it("should stream", function()
    ...
    local seen = {}
    for item in base:Dataset(nil):stream("list", nil, nil) do
    ...
    if type(config.feature) == "table" and config.feature.streaming ~= nil then
      ...
      local got = {}
      for item in streamsdk:Dataset(nil):stream("list", nil, nil) do
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set HONG_KONG_CSDI_TEST_DATASET_ENTID JSON to run live")
      return
    end
...
function dataset_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("HONG_KONG_CSDI_TEST_DATASET_ENTID")
  ...
  if env["HONG_KONG_CSDI_TEST_LIVE"] == "TRUE" then
