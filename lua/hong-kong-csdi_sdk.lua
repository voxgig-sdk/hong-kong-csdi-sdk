...
local HongKongCsdiSDK = {}
HongKongCsdiSDK.__index = HongKongCsdiSDK
...
HongKongCsdiSDK._make_feature = _make_feature


function HongKongCsdiSDK.new(options)
  local self = setmetatable({}, HongKongCsdiSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function HongKongCsdiSDK:options_map()
...
function HongKongCsdiSDK:get_utility()
  return Utility.copy(self._utility)
end


function HongKongCsdiSDK:get_root_ctx()
  return self._rootctx
end


function HongKongCsdiSDK:prepare(fetchargs)
...
function HongKongCsdiSDK:direct(fetchargs)
...
-- by passing it.
function HongKongCsdiSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function HongKongCsdiSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "HongKongCsdiSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function HongKongCsdiSDK:OgcService(data)
  ...
  if data == nil then
    if self._ogc_service == nil then
      self._ogc_service = EntityMod.new(self, nil)
    end
...
function HongKongCsdiSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = HongKongCsdiSDK.new(sdkopts)
  sdk.mode = "test"
