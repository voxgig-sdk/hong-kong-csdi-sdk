# HongKongCsdi Lua SDK

The Lua SDK for the HongKongCsdi API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Dataset(nil):list(nil, nil)
...
local result, err = client:HongKongCsdi(nil):load(
  { id = "test01" }, nil
)
...
```
lua/
├── hong-kong-csdi_sdk.lua    -- Main SDK module
...
The main module (`hong-kong-csdi_sdk`) exports the SDK constructor
