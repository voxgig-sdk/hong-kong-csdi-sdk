...
import (
	...
	omni "github.com/voxgig-sdk/hong-kong-csdi-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.HongKongCsdiError); is {
				out := map[string]any{
					"name":    "HongKongCsdiError",
...
func omniCtx(arg any, client *sdk.HongKongCsdiSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.HongKongCsdiSDK); is {
			client = live
