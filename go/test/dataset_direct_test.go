...
func TestDatasetDirect(t *testing.T) {
	t.Run("direct-list-dataset", func(t *testing.T) {
		setup := datasetDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-dataset", _mode); _shouldSkip {
			if _reason == "" {
...
func datasetDirectSetup(mockres any) *datasetDirectSetupResult {
	...
	env := envOverride(map[string]any{
		"HONG_KONG_CSDI_TEST_DATASET_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["HONG_KONG_CSDI_TEST_DATASET_ENTID"]; ok {
