...
func TestDatasetEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Dataset(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Dataset(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set HONG_KONG_CSDI_TEST_DATASET_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func datasetBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("HONG_KONG_CSDI_TEST_DATASET_ENTID")
	...
	idmapResolved := core.ToMapAny(env["HONG_KONG_CSDI_TEST_DATASET_ENTID"])
