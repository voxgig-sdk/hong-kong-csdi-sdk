...
[MCP](https://modelcontextprotocol.io) server exposing the HongKongCsdi SDK as
two agent tools — `hong-kong-csdi_list` and `hong-kong-csdi_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/hong-kong-csdi-mcp)
make build

# 2. Provide credentials via the environment
export HONG_KONG_CSDI_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user hong-kong-csdi \
  -- /absolute/path/to/hong-kong-csdi-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./hong-kong-csdi-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// hong-kong-csdi_list: first page of records
{ "entity": "dataset" }
{ "entity": "dataset", "query": { } }

// hong-kong-csdi_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/hong-kong-csdi-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user hong-kong-csdi \
     -- "$PWD"/dist/*/hong-kong-csdi-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `hong-kong-csdi_list` and `hong-kong-csdi_load` tools now appear
   in new sessions. Ask the agent to *"list dataset using hong-kong-csdi"*
   and it calls `hong-kong-csdi_list` with `{"entity":"dataset"}`.
...
```sh
export HONG_KONG_CSDI_APIKEY=sk_live_xxx            # API key
export HONG_KONG_CSDI_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `hong-kong-csdi_list` tool
...
### Call the `hong-kong-csdi_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `hong-kong-csdi_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `hong-kong-csdi_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `HONG_KONG_CSDI_APIKEY` | API key sent with every request. |
| `HONG_KONG_CSDI_BASE` | Optional override of the API base URL. |
...
```sh
./hong-kong-csdi-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"hong-kong-csdi_load","arguments":{"entity":"dataset","query":{"id":1}}}}'
```
