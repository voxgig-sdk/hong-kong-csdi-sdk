...
describe('OgcServiceEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['load']) {
      if (maybeSkipControl(t, 'entityOp', 'ogc_service.' + op, live)) return
    }
    ...
    const ogc_service_ref01_data_dt0 = await ogc_service_ref01_ent.load(ogc_service_ref01_match_dt0)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['HONG_KONG_CSDI_TEST_OGC_SERVICE_ENTID']
