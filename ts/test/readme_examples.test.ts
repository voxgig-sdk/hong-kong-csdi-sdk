...
//     (\`new HongKongCsdiSDK(...)\` / \`HongKongCsdiSDK.test(...)\` become
//     \`HongKongCsdiSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'HongKongCsdiSDK'
...
const TEST_SEED = {"entity":{"dataset":{"test01":{"id":"test01"}},"ogc_service":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new HongKongCsdiSDK(...)\` and any
// \`HongKongCsdiSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+HongKongCsdiSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/HongKongCsdiSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
