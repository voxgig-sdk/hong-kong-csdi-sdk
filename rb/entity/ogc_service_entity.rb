...
class OgcServiceEntity
  ...
  def make
    opts = @_entopts.dup
    OgcServiceEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = HongKongCsdiHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [OgcService, Hash] the current OgcService data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = HongKongCsdiHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of OgcService fields)
  def match_get
  ...
  #
  # @param reqmatch [OgcServiceLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.OgcService.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [OgcService, Hash] the loaded OgcService; raises HongKongCsdiError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = HongKongCsdiHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
