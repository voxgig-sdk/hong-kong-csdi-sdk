...
module HongKongCsdiUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = HongKongCsdiResponse.new({})
    ...
    if fetch_err
      ...
      response = HongKongCsdiResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = HongKongCsdiResponse.new(fetched)
    else
