// Command HongKongCsdi-cli is a CLI + REPL for the HongKongCsdi SDK,
...
const prompt = "hong-kong-csdi"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "dataset ogc_service"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      HONG_KONG_CSDI_APIKEY (api key), HONG_KONG_CSDI_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: HONG_KONG_CSDI_APIKEY carries the API key and
	// HONG_KONG_CSDI_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("HONG_KONG_CSDI_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("HONG_KONG_CSDI_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewHongKongCsdiSDK(opts)
