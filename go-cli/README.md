...
boru-driven command-line client **and** interactive REPL for the HongKongCsdi
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/hong-kong-csdi-cli)
make build

# 2. See usage (words, entities, env vars)
./hong-kong-csdi-cli --help
...
./hong-kong-csdi-cli load 1 dataset            # {id:1} shorthand
./hong-kong-csdi-cli load '{id:1}' dataset       # explicit match map
./hong-kong-csdi-cli list ogc_service

# 5. Override the API base URL for a single call
HONG_KONG_CSDI_BASE=https://api.example.com ./hong-kong-csdi-cli list dataset

# 6. No arguments -> interactive REPL
./hong-kong-csdi-cli
hong-kong-csdi> list dataset
hong-kong-csdi> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/hong-kong-csdi-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/hong-kong-csdi-cli list dataset
   ```
...
```sh
./hong-kong-csdi-cli load 1 dataset          # scalar shorthand for {id:1}
./hong-kong-csdi-cli load '{id:1}' dataset     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `hong-kong-csdi>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/hong-kong-csdi-cli`. |
...
(`./hong-kong-csdi-cli <expr>`) and interactively (the REPL), and expressions compose the
